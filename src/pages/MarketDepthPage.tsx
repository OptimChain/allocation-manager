...
function VolSurfaceSection({
  ...
  // One angle shared by every surface, so the three read as comparable views
  // of the same thing rather than three unrelated shapes.
  const [angle, setAngle] = useState(DEFAULT_ANGLE);

  // One box for all three, so they render at a single shared scale and their
  // floors line up across the row.
  ...
  return (
    <section className="space-y-3">
          ...
          {view === 'surface' && (
            <>
              <span className="hidden sm:inline text-xs text-gray-400 dark:text-zinc-500">
                Drag a surface to rotate all three
              </span>
              <button
      ...
      <div
        className={`grid gap-4 ${view === 'surface' ? 'grid-cols-1 lg:grid-cols-3' : 'grid-cols-1'}`}
      >
        {surfaces.map((s) => {
