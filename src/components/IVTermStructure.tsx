...
function extractTermStructure(blob: OptionsChainBlob, snapshotDate: string): ExpiryIV[] {
  ...
  const byExpiry = new Map<string, { ivs: number[]; mids: number[]; strikes: number[] }>();
  ...
  for (const [key, val] of Object.entries(chain)) {
    ...
    if (!byExpiry.has(parsed.expiry)) {
      byExpiry.set(parsed.expiry, { ivs: [], mids: [], strikes: [] });
    }
  ...
  // Also find the underlying mid price from the chain's call/put parity or quote
  // Use all strikes to estimate ATM
  ...
  // Estimate ATM strike as median of traded strikes
  ...
  for (const [expiry, group] of byExpiry) {
    ...
    // ATM IV: find the put closest to the ATM strike
...
/** Check if term structure is inverted (short-dated IV > long-dated IV). */
...
export default function IVTermStructure() {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          ...
          // Auto-select up to 5 most recent dates
  ...
  // Load snapshots for selected dates
  useEffect(() => {
    if (!selectedSymbol || selectedDates.length === 0) return;
    let cancelled = false;
  ...
  // Build chart data: merge all snapshots by DTE
  const chartData = useMemo(() => {
    if (snapshots.length === 0) return [];

    // Collect all unique DTEs
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Loading term structure data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">
                Near-term put IV exceeds longer-dated put IV on{' '}
                <strong>{invertedSnapshots.map((s) => s.date).join(', ')}</strong>.
                ...
                {invertedSnapshots.map((s) => {
                  ...
                  return (
                    <p key={s.date} className="text-xs text-amber-600 dark:text-amber-500">
                      {s.date}: {short.dte}d IV = {shortIV.toFixed(1)}% vs {long.dte}d IV = {longIV.toFixed(1)}%
            ...
            <LineChart data={chartData}>
              ...
              <Tooltip
                ...
                formatter={(value: number, name: string) => {
                  ...
                  const label = snapshots[idx]?.date ?? name;
      ...
      <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-lg p-4">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
          ...
          <p><strong>ATM IV</strong> is estimated from the put strike closest to the median traded strike in the chain. Average IV includes all puts with valid implied volatility.</p>
          <p><strong>Data source:</strong> Alpaca options chain snapshots via Netlify Blobs, polled every ~30 seconds.</p>
        </div>
      </div>
    </div>
  );
